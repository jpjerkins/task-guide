...
Postpone, sharpened what Defer means, and repaired the `Stale` age rule.
...
- **Same gate.** `now >= Postpone` joins the eligibility conjunction under **Defer**, so a postponed
  Task is absent from every match-driven surface exactly as a deferred one is, has no **Scarcity**,
...
Only **eligible** Tasks are ranked, so a deferred or postponed Task has no Scarcity — it is not merely
