...
it('stamping_a_Day_template_onto_a_date_renders_the_copies_the_windows_in_note_and_sends_the_stamp_for_that_date_alone', async () => {
  ...
  await waitFor(() => expect(fetch).toHaveBeenCalledWith('/api/overrides', expect.objectContaining({ method: 'POST', body: JSON.stringify({ from: '2026-11-01', to: '2026-11-01', templateId: template.id, mode: 'stamp' }) })))
...
it('a_range_landing_on_dates_that_already_carry_an_Override_names_every_one_of_them_in_a_single_confirmation_before_the_write_not_one_prompt_per_date', async () => {
  ...
  await waitFor(() => expect(fetch).toHaveBeenCalledWith('/api/overrides', expect.objectContaining({ method: 'POST', body: JSON.stringify({ from: '2026-12-24', to: '2026-12-28', templateId: template.id, mode: 'stamp' }) })))
