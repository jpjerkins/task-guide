...
it('the_title_and_confirm_button_carry_both_counts_when_the_span_exceeds_the_clobbered_count', async () => {
  ...
  act(() => { void confirm(['2026-11-03', '2026-11-04', '2026-11-07'], 12, 'stamp') })
...
it('the_title_is_singular_for_a_single_clobbered_date', async () => {
  render(<Host />)
  const confirm = (window as unknown as { confirm: (affected: readonly string[], span: number, mode: 'stamp' | 'blank') => Promise<boolean> }).confirm
  act(() => { void confirm(['2026-11-03'], 5, 'stamp') })
...
it('the_confirm_button_reads_Replace_all_n_when_the_whole_span_is_clobbered', async () => {
  render(<Host />)
  const confirm = (window as unknown as { confirm: (affected: readonly string[], span: number, mode: 'stamp' | 'blank') => Promise<boolean> }).confirm
  act(() => { void confirm(['2026-11-03', '2026-11-04'], 2, 'stamp') })
...
it('the_list_is_a_damage_block_whose_dates_render_through_fmtShort_with_an_already-an-override_pill_and_the_note_states_the_untouched_count', async () => {
  render(<Host />)
  const confirm = (window as unknown as { confirm: (affected: readonly string[], span: number, mode: 'stamp' | 'blank') => Promise<boolean> }).confirm
  act(() => { void confirm(['2026-11-03', '2026-11-04'], 12, 'stamp') })
...
it('the_untouched-dates_sentence_is_singular_for_exactly_one_untouched_date', async () => {
  render(<Host />)
  const confirm = (window as unknown as { confirm: (affected: readonly string[], span: number, mode: 'stamp' | 'blank') => Promise<boolean> }).confirm
  act(() => { void confirm(['2026-11-03', '2026-11-04'], 3, 'stamp') })
  await screen.findByRole('heading', { name: 'Replace 2 Overrides?' })
...
it('confirming_resolves_true_and_cancelling_resolves_false_and_neither_adds_a_second_cancel', async () => {
  ...
  act(() => { result = confirm(['2026-11-03'], 5, 'stamp') })
...
it('a_single_date_that_is_its_own_whole_span_reads_as_one_date_not_as_a_span_of_one', async () => {
  render(<Host />)
  const confirm = (window as unknown as { confirm: (affected: readonly string[], span: number, mode: 'stamp' | 'blank') => Promise<boolean> }).confirm
  act(() => { void confirm(['2026-11-03'], 1, 'stamp') })
...
// #145: blanking clears the clobbered dates, it does not stamp anything onto them, and the
// untouched dates are cleared too rather than "copied off" the pattern — the wording must say so.
it('blank_mode_never_mentions_stamping_or_copying_off_the_pattern_and_its_button_reads_blank', async () => {
  render(<Host />)
  const confirm = (window as unknown as { confirm: (affected: readonly string[], span: number, mode: 'stamp' | 'blank') => Promise<boolean> }).confirm
  act(() => { void confirm(['2026-11-03', '2026-11-04'], 12, 'blank') })
  const dialog = await screen.findByRole('heading', { name: 'Blank 2 Overrides?' })
  expect(dialog).toBeInTheDocument()
  expect(document.querySelector('.damage-h')).toHaveTextContent(
    '2 of the 12 dates in this span already depart from the pattern. Blanking clears what is on them.')
  expect(document.querySelector('.note')).toHaveTextContent('The other 10 dates are following the pattern and will be cleared too.')
  expect(document.body.textContent).not.toMatch(/stamp|copied off/i)
  expect(screen.getByRole('button', { name: 'Blank all 12' })).toBeInTheDocument()
})

it('blank_mode_for_a_single_date_that_is_its_own_whole_span_reads_Blank_it', async () => {
  render(<Host />)
  const confirm = (window as unknown as { confirm: (affected: readonly string[], span: number, mode: 'stamp' | 'blank') => Promise<boolean> }).confirm
  act(() => { void confirm(['2026-11-03'], 1, 'blank') })
  expect(await screen.findByRole('button', { name: 'Blank it' })).toBeInTheDocument()
  expect(document.querySelector('.damage-h')).toHaveTextContent(
...
it('blank_mode_with_nothing_clobbered_renders_a_count_only_sheet_whose_damage_block_carries_no_rows', async () => {
  ...
  expect(document.body.textContent).toContain('Every window on these dates is removed.')
...
it('blank_mode_with_nothing_clobbered_for_a_single_date_span_reads_Blank_this_date', async () => {
  ...
  expect(document.querySelectorAll('.damage .row')).toHaveLength(0)
  expect(document.body.textContent).toContain('Every window on this date is removed.')
