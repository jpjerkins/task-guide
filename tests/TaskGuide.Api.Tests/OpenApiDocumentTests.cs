...
public sealed class OpenApiDocumentTests : IDisposable
{
    ...
    private static bool HasJsonBody(JsonElement response) =>
        response.TryGetProperty("content", out var content) && content.TryGetProperty("application/json", out _);
    ...
    public async Task Read_handler_declares_200_400_and_404_with_a_typed_200_body(string path, string schemaRef)
    {
        ...
        // #172: the { error } payload is an untyped object, so there is no $ref to pin — the content
        // entry's presence is the assertion. A bodiless arm keeps the status and drops this.
        Assert.True(HasJsonBody(badRequest), "400 carries no application/json body");
        Assert.True(HasJsonBody(notFound), "404 carries no application/json body");
